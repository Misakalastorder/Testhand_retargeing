...
import dataset
from dataset import Normalize, parse_h5, parse_h5_hand, parse_all,parse_arm
...
if __name__ == '__main__':
    ...
    if cfg.INFERENCE.MOTION.KEY:
        ...
        source_q = torch.stack([data.q for data in test_data], dim=0)
        # source_l_joint_pos = torch.stack([data.l_hand_pos for data in test_data], dim=0)
        # source_r_joint_pos = torch.stack([data.r_hand_pos for data in test_data], dim=0)
        g1.create_dataset('l_joint_pos', data=source_pos[:, :3])
        g1.create_dataset('r_joint_pos', data=source_pos[:, 3:])
        g1.create_dataset('l_joint_q', data=source_q[:, :3])
        g1.create_dataset('r_joint_q', data=source_q[:, 3:])
        # g1.create_dataset('l_hand_pos', data=source_l_joint_pos)
        # g1.create_dataset('r_hand_pos', data=source_r_joint_pos)
        hf.close()
        ...
        test_loader = DataListLoader(test_set, batch_size=cfg.HYPER.BATCH_SIZE, shuffle=False, num_workers=0, pin_memory=True)
    ...
    # elbow loss
    el_criterion = nn.MSELoss() if cfg.LOSS.EL else None
    # Create optimizer
    ...
    for epoch in range(cfg.HYPER.EPOCHS):
        train_loss = train_epoch(model, ee_criterion, vec_criterion, col_criterion, lim_criterion, ori_criterion, fin_criterion, reg_criterion,el_criterion, optimizer, test_loader, test_target, epoch, logger, cfg.OTHERS.LOG_INTERVAL, writer, device, z_all)
        # print(z_all)
        if cfg.INFERENCE.MOTION.KEY:
            # Save model
    ...
    if cfg.INFERENCE.MOTION.KEY:
        ...
        for batch_idx, data_list in enumerate(test_loader):
            for target_idx, target in enumerate(test_target):
                ...
                # print("r_hand_pos-------------------------:" , len(r_hand_pos))
        ...
        # print("ang_all-------------------------:", len(ang_all))
        # print("l_hand_ang_all-------------------------:", len(l_hand_ang_all))
        # print("r_hand_ang_all-------------------------:", len(r_hand_ang_all))
        if cfg.INFERENCE.H5.BOOL:
            # hf = h5py.File(os.path.join(cfg.INFERENCE.H5.PATH, 'shenme_0711_dist.h5'), 'w')
            hf = h5py.File(os.path.join(cfg.INFERENCE.H5.PATH, f'{cfg.INFERENCE.MOTION.KEY}_1800.h5'), 'w')
            g1 = hf.create_group('group1')
            if pos_all and ang_all:
                ...
                # print('------',pos[0])
                g1.create_dataset('l_joint_pos', data=pos[:, :6])
                g1.create_dataset('r_joint_pos', data=pos[:, 6:])
                g1.create_dataset('l_joint_angle', data=ang[:, :6])
                g1.create_dataset('r_joint_angle', data=ang[:, 6:])
                # g1.create_dataset('l_joint_pos', data=pos[:, :5])
                # g1.create_dataset('r_joint_pos', data=pos[:, 5:])
                # g1.create_dataset('l_joint_angle', data=ang[:, :5])
                # g1.create_dataset('r_joint_angle', data=ang[:, 5:])
            ...
            #     # print('------',l_hand_angle[0])
            #     r_hand_angle = torch.cat(r_hand_ang_all, dim=0).view(len(test_data), -1).detach().cpu().numpy()
            #     # print('------',r_hand_angle[0])
            #     l_hand_pos = torch.cat(l_hand_pos_all, dim=0).view(len(test_data), -1).detach().cpu().numpy()
            #     # print('------',l_hand_pos[0])
            #     r_hand_pos = torch.cat(r_hand_pos_all, dim=0).view(len(test_data), -1).detach().cpu().numpy()

            #     # remove zeros
            #     l_hand_angle = np.concatenate([l_hand_angle[:,0:3],l_hand_angle[:,4:6],l_hand_angle[:,7:9],l_hand_angle[:,10:12],l_hand_angle[:,13:17]], axis=1)
            #     # print('------', l_hand_angle[0])
            #     r_hand_angle = np.concatenate([r_hand_angle[:,0:3],r_hand_angle[:,4:6],r_hand_angle[:,7:9],r_hand_angle[:,10:12],r_hand_angle[:,13:17]], axis=1)
            #     # print('------', r_hand_angle[0])
